...
describe('SessionPage details flow', () => {
  ...
  it('saves session details and shows them in summary', async () => {
    ...
    await user.type(screen.getByLabelText(/Scheduled/i), '2025-02-10T07:00');
    await user.type(screen.getByLabelText(/Duration \(minutes\)/i), '60');
    await user.type(screen.getByLabelText(/Trainer/i), 'Jordan');
    await user.type(screen.getByLabelText(/Athlete/i), 'Alex');
    await user.type(screen.getByLabelText(/Micro-cycle/i), 'mc-55');
    ...
    expect(await screen.findByText(/Trainer: Jordan/i)).toBeInTheDocument();
    expect(await screen.findByText(/Athlete: Alex/i)).toBeInTheDocument();
    expect(await screen.findByText(/mc-55/i)).toBeInTheDocument();
  });
});
